...
@property(nonatomic,strong)NSMutableArray *pickerArray4;//时

@property(nonatomic,strong)NSMutableArray *pickerArray5;//分

//@property(nonatomic,strong)NSMutableArray *pickerArray6;//秒

@property(nonatomic,strong)UIPickerView *pickerView2; //选择器2

@end

@implementation ESDatePickerViewAllTime
...
-(instancetype)init{
    if (self = [super init]) {
        ...
        self.pickerArray4 = [NSMutableArray array];
        self.pickerArray5 = [NSMutableArray array];
//        self.pickerArray6 = [NSMutableArray array];
        ...
        for (int i = 0; i < 24; i++) {
            [self.pickerArray4 addObject:[NSString stringWithFormat:@"%02d时",i]];
        }
        for (int i = 0; i < 60; i++) {
            [self.pickerArray5 addObject:[NSString stringWithFormat:@"%02d分",i]];
        }
//        for (int i = 0; i < 60; i++) {
//            [self.pickerArray6 addObject:[NSString stringWithFormat:@"%02d秒",i]];
//        }
        [self setDatePicker];
    }
    return self;
...
-(void)setDatePicker{
    ...
    pBgV2.frame = CGRectMake(30, 0, [UIScreen mainScreen].bounds.size.width - 60, 350);
    ...
    self.pickerView2.frame = CGRectMake((pBgV2.bounds.size.width -240) * 0.15, 200,pBgV2.bounds.size.width - (pBgV2.bounds.size.width- 240) *0.30, 150);
    self.pickerView2.backgroundColor = [UIColor whiteColor];
    
    self.pickerView2.dataSource = self;   //这个不用说了瑟
    
    self.pickerView2.delegate = self;       //这个不用说了瑟
    
    self.pickerView2.showsSelectionIndicator = YES;    //这个最好写 你不写来试下哇
    ...
    [self.pickerView2 selectRow:t.hour inComponent:0 animated:NO];
    [self.pickerView2 selectRow:t.minute - 1 inComponent:1 animated:NO];
//    [self.pickerView2 selectRow:t.second - 1 inComponent:2 animated:NO];
    
    //手动选中该行
    [self pickerView:nil didSelectRow:t.month - 1 inComponent:1];
    
    [pBgV2 addSubview:self.pickerView];
    [pBgV2 addSubview:self.pickerView2];
...

{
    ...
    if (component == 0 && pickerView == self.pickerView) {
    ...
    }else if(component ==2 && pickerView == self.pickerView){
        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, 80, 30)];
        ...
        myView.backgroundColor = [UIColor clearColor];
    }else if (component == 0 && pickerView == self.pickerView2) {
        
        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, 80, 30)];
        
        myView.textAlignment = NSTextAlignmentLeft;
        
        myView.text = [self.pickerArray4 objectAtIndex:(row%self.pickerArray4.count)];
    ...
    }else if(component == 1&& pickerView == self.pickerView2){
        
        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, 80, 30)];
        
        myView.text = [self.pickerArray5 objectAtIndex:(row%self.pickerArray5.count)];
...
//    else if(component ==2 && pickerView == self.pickerView2){
//        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, 80, 30)];
//        
//        myView.text = [self.pickerArray6 objectAtIndex:(row%self.pickerArray6.count)];
//        
//        myView.textAlignment = NSTextAlignmentRight;
//        
//        myView.font = [UIFont systemFontOfSize:18];
...
- (void)pickerView:(UIPickerView *)pickerView
    ...
    if (component == 1 && pickerView == self.pickerView) {
        switch (row%12) {
...
-(void)quebClicked{
    ...
    NSInteger h = [self.pickerView2 selectedRowInComponent: 0];
    NSInteger m = [self.pickerView2 selectedRowInComponent: 1];
//    NSInteger s = [self.pickerView2 selectedRowInComponent: 2];
    
    NSString *yearStr = [self.pickerArray1 objectAtIndex: year%5000];
    NSString *monthStr = [self.pickerArray2 objectAtIndex: moon%12];
    NSString *dayStr = @"";
    NSString *hStr = [self.pickerArray4 objectAtIndex: h%24];
    NSString *mStr = [self.pickerArray5 objectAtIndex: m%60];
//    NSString *sStr = [self.pickerArray6 objectAtIndex: s%60];
    switch (moon%12) {
        case 0:case 2:case 4:case 6:case 7:case 9:case 11:
        {
    ...
    hStr = [hStr substringToIndex:hStr.length-1];
    mStr = [mStr substringToIndex:mStr.length-1];
//    sStr = [sStr substringToIndex:sStr.length-1];
    
    
    if ([self.delegate respondsToSelector:@selector(ESDPGetYear:month:day:hour:minute:second:)]) {
        [self.delegate ESDPGetYear:yearStr month:monthStr day:dayStr hour:hStr minute:mStr second:@"00"];
    }
